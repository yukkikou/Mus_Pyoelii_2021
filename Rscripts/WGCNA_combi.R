...
expr_bi0 <- t(as.matrix(CombiGeneFt))
expr.pca_bi0 <- PCA(expr_bi0, ncp = 2, scale.unit = TRUE, graph = F)
plot(expr.pca_bi0) # 4.5*3.8

datExpr_bi0 = CombiGeneFt
datExpr_bi0 <- t(datExpr_bi0)


# check missing and outlier
gsg = goodSamplesGenes(datExpr_bi0, verbose = 3)
gsg$allOK
...
sampleTree = hclust(dist(datExpr_bi0), method = "average")
...
keepSamples = (clust==0)
datExpr_bi = datExpr_bi0[keepSamples, ]
nGenes = ncol(datExpr_bi)
nSamples = nrow(datExpr_bi)
...
biSamples = rownames(datExpr_bi)
traitRows = match(biSamples, allTraits$Sample)
...
#Re-cluster samples
sampleTree2 = hclust(dist(datExpr_bi), method = "average")
...
save(datExpr_bi, datTraits, file = "figure/combination//FemaleLiver-01-dataInput.RData")


#2.a Automatic network construction and module detection
########################################################################step 1: Choosing the soft-thresholding power
lnames = load(file = "figure/combination//FemaleLiver-01-dataInput.RData")
lnames
# Choose a set of soft-thresholding powers
powers = c(c(1:10), seq(from = 12, to=20, by=2))
# Call the network topology analysis function
sft = pickSoftThreshold(datExpr_bi, powerVector = powers, verbose = 5)
# Plot the results:
sizeGrWindow(9, 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
softPower = 8
adjacency = adjacency(datExpr_bi, power = softPower)
...
MEList = moduleEigengenes(datExpr_bi, colors = dynamicColors)
...
#as figure shows
MEDissThres = 0.1   ###change point###

# Plot the cut line into the dendrogram
abline(h=MEDissThres, col = "red")
# Call an automatic merging function
merge = mergeCloseModules(datExpr_bi, dynamicColors, cutHeight = MEDissThres, verbose = 3)
...
save(MEs, moduleLabels, moduleColors, geneTree, file = "figure/combination/FemaleLiver-02-networkConstruction-stepByStep.RData")
...
lnames = load(file = "figure/combination/FemaleLiver-02-networkConstruction-stepByStep.RData")
lnames
# Define numbers of genes and samples
nGenes = ncol(datExpr_bi);
nSamples = nrow(datExpr_bi);
# Recalculate MEs with color labels
MEs0 = moduleEigengenes(datExpr_bi, moduleColors)$eigengenes
MEs = orderMEs(MEs0)
...
geneModuleMembership = as.data.frame(cor(datExpr_bi, MEs, use = "p"))
MMPvalue = as.data.frame(corPvalueStudent(as.matrix(geneModuleMembership), nSamples))

names(geneModuleMembership) = paste("MM", modNames, sep="")
names(MMPvalue) = paste("p.MM", modNames, sep="")
geneTraitSignificance = as.data.frame(cor(datExpr_bi, condition, use = "p"))
...
dissTOM = 1-TOMsimilarityFromExpr(datExpr_bi, power = 20)
...
MEs = moduleEigengenes(datExpr_bi, moduleColors)$eigengenes
...
probes = colnames(datExpr_bi)
hiCorYoeliiModule = highCorModule %>% 
  filter(abs(cor) >= 0.8) %>% 
  arrange(-abs(cor)) %>%
  separate(module, c("prefix", "color"), "E") %>%
  select(color, cor) %>%
  filter(color %in% unique(moduleColors[probes %in% rownames(CombiPyGeneTPM)]))

hiCorYoeliiModule$color
# [1] "black"   "magenta" "pink"    "cyan"    "brown" 
...
# [1] "black"   "magenta" "pink"    "cyan"    "brown"
nTop = 1000
IMConn = softConnectivity(datExpr_bi[, modProbes])
top = (rank(-IMConn) <= nTop)
topmod <- modTOM[top, top]
hiYoeGene = rownames(topmod)[rownames(topmod) %in% rownames(CombiPyGeneTPM)]
write.table(hiYoeGene, paste("figure/combination/HighYoeliiGene-", 
                             paste(modules, collapse="-"), ".txt", sep=""), quote = F, col.names = F, row.names = F)


# Export the network into edge and node list files Cytoscape can read
cyt = exportNetworkToCytoscape(modTOM,
                               edgeFile = paste("figure/combination/CytoscapeInput-edges-", paste(modules, collapse="-"), ".txt", sep=""),
                               nodeFile = paste("figure/combination/CytoscapeInput-nodes-", paste(modules, collapse="-"), ".txt", sep=""),
                               weighted = TRUE,
                               threshold = 0.3,
                               nodeNames = modProbes,
...
ori = read_tsv("figure/WGCNA/HighYoeliiGene-Module.txt", col_names = F)
new = read_tsv("figure/combination/HighYoeliiGene.txt", col_names = F)
table(new$X1 %in% ori$X1)
